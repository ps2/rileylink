...
static uint8_t cmdGetByte() reentrant {
  Packet *packet;
  uint8_t rval = 0;
  if (packetCount > 0)
  {
...
static void doCommand(uint8_t cmd) reentrant {
  lastCmd = cmd;
  switch (cmd) {
...
void setRXChannel(uint8_t newChannel) reentrant {
...
static void addDecodedByte(uint8_t value) reentrant {
...
static void finishIncomingPacket() reentrant {
  // Compute crc
...
void receiveRadioSymbol(uint8_t value) reentrant {
